module ActionDispatch
  module Assertions
    module SelectorAssertions
      # Selects content from a Kissy response.  Patterned loosely on
      # assert_select_rjs.
      #
      # === Narrowing down
      #
      ...
      # === Using blocks
      #
      # Without a block, +assert_select_kissy_ merely asserts that the
      # response contains one or more statements that match the conditions
      # specified above
      #
      # With a block +assert_select_kissy_ also asserts that the method call
      ...
      def assert_select_kissy(*args, &block)
        kissy_method = args.first.is_a?(Symbol) ? args.shift : nil
        kissy_opt    = args.first.is_a?(Symbol) ? args.shift : nil
        id            = args.first.is_a?(String) ? args.shift : nil

        pattern = "\\.#{kissy_method || '\\w+'}\\("
        pattern = "#{pattern}['\"]#{kissy_opt}['\"],?\\s*" if kissy_opt
        pattern = "#{pattern}#{PATTERN_HTML}"
        pattern = "(?:Kissy|\\$)\\(['\"]#{id}['\"]\\)#{pattern}" if id
        ...
        if fragments.empty?
          opts = [kissy_method, kissy_opt, id].compact
          flunk "No Kissy call matches #{opts.inspect}"
        end
